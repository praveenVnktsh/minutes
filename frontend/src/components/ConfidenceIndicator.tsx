...
export const ConfidenceIndicator: React.FC<ConfidenceIndicatorProps> = ({
  ...
  const getColorClass = (conf: number): string => {
    if (conf >= 0.8) return toneFill.success; // 80-100%: High confidence
    if (conf >= 0.7) return 'bg-success/60'; // 70-79%: Good confidence
    if (conf >= 0.4) return toneFill.warning; // 40-69%: Medium confidence
    return toneFill.error; // Below 40%: Low confidence
  };
